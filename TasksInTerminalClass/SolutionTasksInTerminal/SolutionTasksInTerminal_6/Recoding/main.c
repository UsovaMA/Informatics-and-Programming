...
void Memory(int** matrix, int STR, int STLB)           //Функция выделения памяти для массива(не знаю почему, но не работает)
{
...
void Filling_vector(int * vector)                      //Функция для заполнения массива с вектором
{
    for (int i = 0; i < M; i++)
    {
        vector[i] = rand() % 10;
...
void Filling_matrix(int **matrix, int STR, int STLB)   //Функция для заполения массива с матрицей      
{
    int max = 4, min = 0;
    for (int i = 0; i < STR; i++) 
    {
