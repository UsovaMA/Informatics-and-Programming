...
int main()
{
	...
	int* crand = NULL, leght = 0, user_num = 0, digit = 0, * user_numarr = NULL, user_numfarr = 0, f = 0, cows = 0, bulls = 0;
	...
	if (leght >= 2 && leght <= 5)
	{
		...
		while(bulls != leght)
		{
			...
			{
				printf("\nПохоже вы ввели неподходящее число(больше загадываемого или содержащее ноль) пожалуйста попробуйте ещё раз...");
			}
			digit = 0;
	    }
		printf("\nВы угадали число!!!");
